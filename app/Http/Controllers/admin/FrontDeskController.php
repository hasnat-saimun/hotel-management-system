...
use App\Services\RoomRackActionService;
...
class FrontDeskController extends Controller
{
    ...
    public function roomRackRoomDetails(Request $request, Room $room, RoomRackActionService $roomRackActions)
    {
        $rackDate = $roomRackActions->rackDateFromRequest($request);

        return response()->json([
            'ok' => true,
            'room' => $roomRackActions->roomDetails($room, $rackDate),
        ]);
    }

    public function roomRackCheckIn(Request $request, Room $room, RoomRackActionService $roomRackActions)
    {
        $rackDate = $roomRackActions->rackDateFromRequest($request);

        try {
            $message = $roomRackActions->checkIn($room, $rackDate);
        } catch (\Throwable $e) {
            return response()->json([
                'ok' => false,
    ...
    public function roomRackCheckOut(Room $room, RoomRackActionService $roomRackActions)
    {
        try {
            $message = $roomRackActions->checkOut($room);
        } catch (\Throwable $e) {
            return response()->json([
                'ok' => false,
                'message' => $e->getMessage(),
    ...
    public function roomRackUpdateHousekeeping(Request $request, Room $room, RoomRackActionService $roomRackActions)
    {
        $data = $request->validate([
            'state' => ['required', Rule::in(['dirty', 'clean', 'maintenance', 'available'])],
        ]);

        try {
            $message = $roomRackActions->updateHousekeeping($room, (string) $data['state']);
        } catch (\Throwable $e) {
            return response()->json([
                'ok' => false,
                'message' => $e->getMessage(),
    ...
    public function roomRackBlock(Room $room, RoomRackActionService $roomRackActions)
    {
        try {
            $message = $roomRackActions->block($room);
        } catch (\Throwable $e) {
            return response()->json([
                'ok' => false,
                'message' => $e->getMessage(),
    ...
    public function roomRackUnblock(Room $room, RoomRackActionService $roomRackActions)
    {
        try {
            $message = $roomRackActions->unblock($room);
        } catch (\Throwable $e) {
            return response()->json([
                'ok' => false,
                'message' => $e->getMessage(),
